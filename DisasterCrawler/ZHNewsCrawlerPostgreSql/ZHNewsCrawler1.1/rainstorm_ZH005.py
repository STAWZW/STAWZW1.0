...
# ---------------------------------------------------------------
def rainstorm_ZH005():
    result = spiderConfig.spiderConfig('rainstorm_ZH005')
    if result['status'] == '1':
        ...
        try:
            url = 'https://search.sina.com.cn/?q=%B1%A9%D3%EA&range=title&c=news&sort=time'
            infos_paser(url,delay)
        except Exception as e:
            print("rainstorm_ZH005访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,rainstorm_ZH005)
        timrFor.start()
    else:
        print('rainstorm_ZH005爬虫停止')
        timrFor = Timer(1*60*60,rainstorm_ZH005)
        timrFor.start()
