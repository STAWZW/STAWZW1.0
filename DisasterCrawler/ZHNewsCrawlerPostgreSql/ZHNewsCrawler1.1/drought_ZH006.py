...
def drought_ZH006():
    result = spiderConfig.spiderConfig('drought_ZH006')
    if result['status'] == '1':
        ...
        try:
            url = 'http://www.tianqi.com/tag/%B8%C9%BA%B5/'
            infos_paser(url,delay)
        except Exception as e:
            print("drought_ZH006访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,drought_ZH006)
        timrFor.start()
    else:
        print('drought_ZH006爬虫停止')
        timrFor = Timer(1*60*60,drought_ZH006)
        timrFor.start()
