...
# ---------------------------------------------------------------
def rainstorm_ZH007():
    result = spiderConfig.spiderConfig('rainstorm_ZH007')
    if result['status'] == '1':
        ...
        try:
            url = 'https://search.cctv.com/search.php?qtext=%E6%9A%B4%E9%9B%A8&type=web'
            infos_paser(url,delay)
        except Exception as e:
            print("rainstorm_ZH007访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,rainstorm_ZH007)
        timrFor.start()
    else:
        print('rainstorm_ZH007爬虫停止')
        timrFor = Timer(1*60*60,rainstorm_ZH007)
        timrFor.start()
