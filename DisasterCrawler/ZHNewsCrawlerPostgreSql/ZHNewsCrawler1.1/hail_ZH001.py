...
def hail_ZH001():
    result = spiderConfig.spiderConfig('hail_ZH001')
    if result['status'] == '1':
        ...
        try:
            url = 'http://www.tianqi.com/tag/%B1%F9%B1%A2/'
            infos_paser(url,delay)
        except Exception as e:
            print("hail_ZH001访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,hail_ZH001)
        timrFor.start()
    else:
        print('hail_ZH001爬虫停止')
        timrFor = Timer(1*60*60,hail_ZH001)
        timrFor.start()
