...
def frost_ZH001():
    result = spiderConfig.spiderConfig('frost_ZH001')
    if result['status'] == '1':
        ...
        try:
            url = 'http://www.tianqi.com/tag/%CB%AA%B6%B3/'
            infos_paser(url,delay)
        except Exception as e:
            print("frost_ZH001访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,frost_ZH001)
        timrFor.start()
    else:
        print('frost_ZH001爬虫停止')
        timrFor = Timer(1*60*60,frost_ZH001)
        timrFor.start()
