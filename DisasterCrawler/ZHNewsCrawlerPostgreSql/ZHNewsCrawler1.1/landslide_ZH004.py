...
def landslide_ZH004():
    result = spiderConfig.spiderConfig('landslide_ZH004')
    if result['status'] == '1':
        ...
        try:
            url = 'https://search.sina.com.cn/?q=%C9%BD%CC%E5%BB%AC%C6%C2&range=title&c=news&sort=time'
            infos_paser(url,delay)
        except Exception as e:
            print("landslide_ZH004访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,landslide_ZH004)
        timrFor.start()
    else:
        print('landslide_ZH004爬虫停止')
        timrFor = Timer(1*60*60,landslide_ZH004)
        timrFor.start()
