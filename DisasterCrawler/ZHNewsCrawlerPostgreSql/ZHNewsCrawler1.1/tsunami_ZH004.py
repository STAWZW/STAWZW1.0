...
def tsunami_ZH004():
    result = spiderConfig.spiderConfig('tsunami_ZH004')
    if result['status'] == '1':
        ...
        try:
            url = 'https://search.sina.com.cn/?q=%BA%A3%D0%A5&range=title&c=news&sort=time'
            infos_paser(url,delay)
        except Exception as e:
            print("tsunami_ZH004访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,tsunami_ZH004)
        timrFor.start()
    else:
        print('tsunami_ZH004爬虫停止')
        timrFor = Timer(1*60*60,tsunami_ZH004)
        timrFor.start()
