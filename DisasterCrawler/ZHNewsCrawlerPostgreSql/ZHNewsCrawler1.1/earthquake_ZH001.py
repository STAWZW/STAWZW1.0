...
def place(newPlace):
        ...
        placeStr = placeVlue[0][0] + placeVlue[0][1] + placeVlue[0][2]
        if placeStr != '':
            ...
            country = ''
            place = placeStr
        return province,country,place

#---------------------------------------------------------------

def earthquake_ZH001():
    result = spiderConfig.spiderConfig('earthquake_ZH001')
    if result['status'] == '1':
        ...
        timrFor = Timer(frequency*60*60,earthquake_ZH001)
        timrFor.start()
    else:
        print('earthquake_ZH001爬虫停止')
        timrFor = Timer(1*60*60,earthquake_ZH001)
        timrFor.start()
