...
def placeMany(originalText):
    ...
    fprovinceList = []
    fcountryList = []
    words, ners = fool.analysis(originalText)                                           #foolnltk自然语言处理工具对文本内容进行标注
    for itemSun in ners[0]:
        if itemSun[2] == 'location':                                                    #提取地点词
    ...
    for item in placeList:
        location_str = [item]
        dfList = cpca.transform(location_str, cut=False)                                #利用地理编码库对地点进行编码，获取完整的地理位置信息
        ...
        if addPlace != '':
            ...
            domesticList.append('中国' + addPlace)
        else:
...
def Deduplication(placeList, provinceList, countryList):                                #地点去重
    ccountry = []
    cprovince = []
    cplaceList = []
    count = 0
...
def latlngFunction(setList):                                                            #经纬度匹配
    placeList = []
    provinceList = []
    countryList = []
    latList = []
    ...
    for item1,item2,item3 in zip(setList[0], setList[1], setList[2]):
        if item3 == '中国':
            lnglat = geocode(item1)                                                     #调用高德地图API获取国内地点经纬度
