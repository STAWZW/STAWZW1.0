...
def analyzeInfo(item):
    ...
    result['disasterid'] = '1000101'                                            #灾害类型
...
def avalanche_ZH001():
    result = spiderConfig.spiderConfig('avalanche_ZH001')
    if result['status'] == '1':
        ...
        try:
            url = 'http://www.tianqi.com/tag/%D1%A9%B1%C0/'
            infos_paser(url,delay)
        except Exception as e:
            print("avalanche_ZH001访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,avalanche_ZH001)
        timrFor.start()
    else:
        print('avalanche_ZH001爬虫停止')
        timrFor = Timer(1*60*60,avalanche_ZH001)
        timrFor.start()
