...
def analyzeInfo_One(item):
    ...
    resultSunSub = {}
    resultSunSub['title'] = result['title']
    resultSunSub['originalText'] = result['originalText']
    resultSunSub['pictures'] = result['pictures']
    
    try:
        title = 'drought_ZH002'
        res = postgreCommand.insertData(result,resultSunSub,title)
        if res == 1:
            print(title,'数据插入成功！')
        elif res == 0:
            print(title,'数据更新成功！')
...
def drought_ZH002():
    result = spiderConfig.spiderConfig('drought_ZH002')
    if result['status'] == '1':
        ...
        try:
            url = 'http://www.qxkp.net/zhfy/ghzh/'
            infos_paser(url,delay)
        except Exception as e:
            print("drought_ZH002访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,drought_ZH002)
        timrFor.start()
    else:
        print('drought_ZH002爬虫停止')
        timrFor = Timer(1*60*60,drought_ZH002)
        timrFor.start()
