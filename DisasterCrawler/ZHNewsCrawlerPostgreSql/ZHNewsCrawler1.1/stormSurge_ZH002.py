...
# ---------------------------------------------------------------
def stormSurge_ZH002():
    result = spiderConfig.spiderConfig('stormSurge_ZH002')
    if result['status'] == '1':
        ...
        try:
            url = 'https://www.baidu.com/s?tn=news&rtt=4&bsst=1&cl=2&wd=%E9%A3%8E%E6%9A%B4%E6%BD%AE&medium=0'
            infos_paser(url,delay)
        except Exception as e:
            print("stormSurge_ZH002访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,stormSurge_ZH002)
        timrFor.start()
    else:
        print('stormSurge_ZH002爬虫停止')
        timrFor = Timer(1*60*60,stormSurge_ZH002)
        timrFor.start()
