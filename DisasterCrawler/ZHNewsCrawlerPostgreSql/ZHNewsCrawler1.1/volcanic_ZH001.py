...
def volcanic_ZH001():
    result = spiderConfig.spiderConfig('volcanic_ZH001')
    if result['status'] == '1':
        ...
        try:
            url = 'http://www.tianqi.com/tag/%BB%F0%C9%BD%B1%AC%B7%A2/'
            infos_paser(url,delay)
        except Exception as e:
            print("volcanic_ZH001访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,volcanic_ZH001)
        timrFor.start()
    else:
        print('volcanic_ZH001爬虫停止')
        timrFor = Timer(1*60*60,volcanic_ZH001)
        timrFor.start()
