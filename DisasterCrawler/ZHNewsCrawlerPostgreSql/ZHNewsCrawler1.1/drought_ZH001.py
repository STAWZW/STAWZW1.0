...
def analyzeInfo(item):
    ...
    result['disasterid'] = '10112'                                          #新闻类别:干旱
...
def drought_ZH001():
    result = spiderConfig.spiderConfig('drought_ZH001')
    if result['status'] == '1':
        ...
        try:
            url = 'https://search.cctv.com/search.php?qtext=%E5%B9%B2%E6%97%B1&type=web'
            infos_paser(url,delay)
        except Exception as e:
            print("drought_ZH001访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,drought_ZH001)
        timrFor.start()
    else:
        print('drought_ZH001爬虫停止')
        timrFor = Timer(1*60*60,drought_ZH001)
        timrFor.start()
# --------测试代码-----------
#drought_ZH001()
