...
def landslide_ZH005():
    result = spiderConfig.spiderConfig('landslide_ZH005')
    if result['status'] == '1':
        ...
        try:
            url = 'http://www.tianqi.com/tag/%C9%BD%CC%E5/'
            infos_paser(url,delay)
        except Exception as e:
            print("landslide_ZH005访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,landslide_ZH005)
        timrFor.start()
    else:
        print('landslide_ZH005爬虫停止')
        timrFor = Timer(1*60*60,landslide_ZH005)
        timrFor.start()
