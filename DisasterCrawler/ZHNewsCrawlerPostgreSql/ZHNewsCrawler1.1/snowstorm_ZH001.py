...
def snowstorm_ZH001():
    result = spiderConfig.spiderConfig('snowstorm_ZH001')
    if result['status'] == '1':
        ...
        try:
            url = 'http://www.tianqi.com/tag/%B1%A9%B7%E7%D1%A9/'
            infos_paser(url,delay)
        except Exception as e:
            print("snowstorm_ZH001访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,snowstorm_ZH001)
        timrFor.start()
    else:
        print('snowstorm_ZH001爬虫停止')
        timrFor = Timer(1*60*60,snowstorm_ZH001)
        timrFor.start()
