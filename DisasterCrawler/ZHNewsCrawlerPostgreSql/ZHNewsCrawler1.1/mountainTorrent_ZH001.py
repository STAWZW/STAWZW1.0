...
def mountainTorrent_ZH001():
    result = spiderConfig.spiderConfig('mountainTorrent_ZH001')
    if result['status'] == '1':
        ...
        try:
            url = 'http://www.tianqi.com/tag/%C9%BD%BA%E9/'
            infos_paser(url,delay)
        except Exception as e:
            print("mountainTorrent_ZH001访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,mountainTorrent_ZH001)
        timrFor.start()
    else:
        print('mountainTorrent_ZH001爬虫停止')
        timrFor = Timer(1*60*60,mountainTorrent_ZH001)
        timrFor.start()

# --------测试代码-----------
#mountainTorrent_ZH001()
