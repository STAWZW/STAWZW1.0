...
def disasterNB(text):
    result = {'雪崩': '1000101', '冰崩': '1000102', '岩崩': '1000103', '岩石崩塌': '1000103', '山体崩塌': '1000104','山体': '1000104'}
    collapseList = ['雪崩','冰崩','岩崩','岩石崩塌','山体崩塌','山体']
    itemIndex = 6
...
def collapse_ZH002():
    result = spiderConfig.spiderConfig('collapse_ZH002')
    if result['status'] == '1':
        ...
        try:
            url = 'http://www.tianqi.com/tag/%C9%BD%CC%E5%CB%FA%B7%BD/'
            infos_paser(url,delay)
        except Exception as e:
            print("collapse_ZH002访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,collapse_ZH002)
        timrFor.start()
    else:
        print('collapse_ZH002爬虫停止')
        timrFor = Timer(1*60*60,collapse_ZH002)
        timrFor.start()
