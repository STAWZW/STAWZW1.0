...
def tsunami_ZH005():
    result = spiderConfig.spiderConfig('tsunami_ZH005')
    if result['status'] == '1':
        ...
        try:
            url = 'https://search.cctv.com/search.php?qtext=%E6%B5%B7%E5%95%B8&type=web'
            infos_paser(url,delay)
        except Exception as e:
            print("tsunami_ZH005访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,tsunami_ZH005)
        timrFor.start()
    else:
        print('tsunami_ZH005爬虫停止')
        timrFor = Timer(1*60*60,tsunami_ZH005)
        timrFor.start()
