...
# ---------------------------------------------------------------
def forestFire_ZH002():
    result = spiderConfig.spiderConfig('forestFire_ZH002')
    if result['status'] == '1':
        ...
        try:
            url = 'https://www.baidu.com/s?ie=utf-8&cl=2&medium=0&rtt=1&bsst=1&rsv_dl=news_t_sk&tn=news&word=%E6%A3%AE%E6%9E%97%E7%81%AB%E7%81%BE&rsv_sug3=22&rsv_sug4=626&rsv_sug1=16&rsv_sug2=0&inputT=7419'
            infos_paser(url,delay)
        except Exception as e:
            print("forestFire_ZH002访问网站失败", str(e))
        postgreCommand.closePostgre()
        timrFor = Timer(frequency*60*60,forestFire_ZH002)
        timrFor.start()
    else:
        print('forestFire_ZH002爬虫停止')
        timrFor = Timer(1*60*60,forestFire_ZH002)
        timrFor.start()
