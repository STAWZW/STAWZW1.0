...
# ---------------------------------------------------------------
def forestFire_ZH003():
    global postgreCommand
    postgreCommand = PostgreCommand()
    postgreCommand.connectPostgre()
    url = 'http://so.dzwww.com/web/search?searchscope=DOCTITLE&timescope=&timescopecolumn=&orderby=LIFO&channelid=205667&andsen=&total=&orsen=&exclude=&searchword=%E6%A3%AE%E6%9E%97%E7%81%AB%E7%81%BE&perpage=&templet=&token=&timeline='
    link_num = 15
    for item in range(link_num):
        try:
            url = infos_paser(url)
        except Exception as e:
            print("forestFire_ZH003访问网站失败", str(e))
            continue
