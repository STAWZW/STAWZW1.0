...
def rainstorm_ZH004():
    ...
    url = 'http://so.dzwww.com/web/search?searchscope=DOCTITLE&timescope=&timescopecolumn=&orderby=LIFO&channelid=205667&andsen=&total=&orsen=&exclude=&searchword=%E6%9A%B4%E9%9B%A8&perpage=&templet=&token=&timeline='
    link_num = 15
    for item in range(link_num):
        try:
            url = infos_paser(url)
        except Exception as e:
            print("rainstorm_ZH004访问网站失败", str(e))
            continue
