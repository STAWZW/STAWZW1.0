...
def infos_paser(url):
    ...
    for item in table_info_list:
        try:
            ...
            continue
    div_info_list = soup.find_all('div', attrs={'id': 'outlinebar'}, limit=1)
    a_info_list = div_info_list[0].find_all('a', attrs={'class': 'next-page'})
    link = 'http://so.dzwww.com/web/' + a_info_list[0]['href']
    url = link.split('×')
    return url[0]
...
def analyzeInfo(item):
    ...
    result['link'] = a_list[0].get_text().strip()
    originalList = get_original(result['link'])
    result['title'] = originalList[0]
    result['source'] = originalList[1]
    result['releaseTime'] = originalList[2]
    result['originalText'] = originalList[3]
    result['disasterid'] = '1000104'
    result['pictures'] = originalList[4]
...
def collapse_ZH004():
    ...
    url = 'http://so.dzwww.com/web/search?searchscope=DOCTITLE&timescope=&timescopecolumn=&orderby=LIFO&channelid=205667&andsen=&total=&orsen=&exclude=&searchword=%E5%B1%B1%E4%BD%93%E5%B4%A9%E5%A1%8C&perpage=&templet=&token=&timeline='
    link_num = 20
    for item in range(link_num):
        try:
            url = infos_paser(url)
        except Exception as e:
            print("collapse_ZH004访问网站失败", str(e))
            continue
