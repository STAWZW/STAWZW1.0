...
def analyzeInfo(item):
    ...
    result['disasterid'] = disasterNB(originalText)                             #灾害类型
...
# ---------------------------------------------------------------
def debrisFlow_ZH003():
    global postgreCommand
    postgreCommand = PostgreCommand()
    postgreCommand.connectPostgre()
    url = 'http://so.dzwww.com/web/search?searchscope=DOCTITLE&timescope=&timescopecolumn=&orderby=LIFO&channelid=205667&andsen=&total=&orsen=&exclude=&searchword=%E6%B3%A5%E7%9F%B3%E6%B5%81&perpage=&templet=&token=&timeline='
    link_num = 15
    for item in range(link_num):
        try:
            url = infos_paser(url)
        except Exception as e:
            print("debrisFlow_ZH003访问网站失败", str(e))
            continue
