...
# ---------------------------------------------------------------
def stormSurge_ZH003():
    global postgreCommand
    postgreCommand = PostgreCommand()
    postgreCommand.connectPostgre()
    url = 'http://so.dzwww.com/web/search?searchscope=DOCTITLE&timescope=&timescopecolumn=&orderby=LIFO&channelid=205667&andsen=&total=&orsen=&exclude=&searchword=%E9%A3%8E%E6%9A%B4%E6%BD%AE&perpage=&templet=&token=&timeline='
    link_num = 15
    for item in range(link_num):
        try:
            url = infos_paser(url)
        except Exception as e:
            print("stormSurge_ZH003访问网站失败", str(e))
            continue
