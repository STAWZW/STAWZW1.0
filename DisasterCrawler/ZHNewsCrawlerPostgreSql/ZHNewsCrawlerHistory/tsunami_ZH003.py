...
def tsunami_ZH003():
    ...
    url = 'http://so.dzwww.com/web/search?searchscope=DOCTITLE&timescope=&timescopecolumn=&orderby=LIFO&channelid=205667&andsen=&total=&orsen=&exclude=&searchword=%E6%B5%B7%E5%95%B8&perpage=&templet=&token=&timeline='
    link_num = 15
    for item in range(link_num):
        try:
            url = infos_paser(url)
        except Exception as e:
            print("tsunami_ZH003访问网站失败", str(e))
            continue
