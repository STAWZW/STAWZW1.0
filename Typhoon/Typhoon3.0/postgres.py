...
class PostgreCommand(object):
    ...
    def allHistorySelectData(self):
        try:
            #sqlExit = "SELECT typhoon_id FROM typhooninfo WHERE typhoon_year = '2015'"     #近五年的所有数据id,由于数据过多，现只取19年的
    ...
    def deletPreviewInsertData(self, typhoon_id, address):
        sqlDelet = "DELETE FROM previewdatas WHERE typhoon_id = '%s' AND forcast_country_name = '%s'" % (typhoon_id,address)
        try:
